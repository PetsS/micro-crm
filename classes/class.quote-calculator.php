...
class QuoteCalculator
{
    ...
    public function calculateResultsFromTransient($form_data)
    {
        // Initialize variables
        $total_tva = 0;
        $total_ht = 0;
        ...
        for ($i = 0; $i < count($form_data['nbPersons']); $i++) {
            $age_data = getAgeById($form_data['ages'][$i]);
            $total_paying_persons += $age_data === '1' ? 0 : $form_data['nbPersons'][$i]; // update total number of paying person, excluding the age category 1 (age less than 3 years old)
            $total_persons += $form_data['nbPersons'][$i]; // update total number of persons
        }
        
        // Calculate prices
        for ($i = 0; $i < count($form_data['nbPersons']); $i++) {
            $age_data = getAgeById($form_data['ages'][$i]); // get one row of age data in the current quote
            $nbPersons_data = $form_data['nbPersons'][$i];
            ...
            $unit_ht[] = ($unit_ttc / (1 + (TVA / 100))); // one unit price without tax
            $amount_ht[] = ($unit_ttc / (1 + (TVA / 100))) * ($nbPersons_data); // full price based on the number of person without tax
            $amount_ttc[] = $unit_ttc * $nbPersons_data; // full price based on the number of person with tax
            $amount_tva = ($unit_ttc / 100) * TVA * $nbPersons_data; // full amount of the tax
        ...
        // Calculate guided price
        $visitetype_guided = getVisiteTypeById($form_data['visitetype']); // run a query in the database to get the guided category row
